...
import configparser
from data_generator import FACE_SIZE, MAX_TEXT_FEATURE
from utils import MultiModalLoss, MoseiEmotionLoss
from tools.model_utils import get_attention_adjusted


def parse_config(feature_type, last_layer=False):
    ...
    ret.append(dropout)
    if last_layer:
        ret.append(config['category'].get('category'))
    else:
        ret.append('emo')
    return ret
...
def audio_model(args, num_classes=7):
    ...
    attention_pool, temporal_layers, act, dropout, last_layer_postfix = parse_config('audio', last_layer=True)
    ...
    x = layers.Dense(units=num_classes, activation=act, name='audio_{}'.format(last_layer_postfix))(x)
...
def video_model(args, num_classes=7):
    ...
    attention_pool, temporal_layers, act, model, dropout, last_layer_postfix = parse_config('video', last_layer=True)
    ...
    x = layers.Dense(units=num_classes, activation=act, name='face_{}'.format(last_layer_postfix))(x)
...
def text_model(args, num_classes=7):
    ...
    attention_pool, temporal_layers, act, MAX_LEN, dropout, last_layer_postfix = parse_config('text', last_layer=True)

    text_input = layers.Input(shape=(MAX_LEN, MAX_TEXT_FEATURE), name=prefix[:-1])
    ...
    x = layers.Dense(units=num_classes, activation=act, name='text_{}'.format(last_layer_postfix))(x)
...
def multimodal_model(args, num_classes=7):
    ...
    :return:
    """
    base_audio_model = audio_model(args, num_classes=num_classes)
    base_video_model = video_model(args, num_classes=num_classes)
    base_text_model = text_model(args, num_classes=num_classes)
    ...
    last_layer_postfix = config['category'].get('category')
    ...
    mm_feat = tf.stack([audio_feat, video_feat, text_feat], axis=2)  # Batch size x 32 x 3  # text_feat

    mm_feat = get_attention_adjusted(mm_feat, 3, prefix='mm', permute=False)   # num_feat = 3 - default
    # mm_feat = get_attention_adjusted(mm_feat, 2, prefix='mm', permute=False)  # num_feat = 3 - default
    ...
    # feat_dict = {'text': base_text_model.input, 'video': base_video_model.input,}
    if multi_outputs:
        mm_output = layers.Dense(num_classes, activation=act)(mm_feat)

        in_out = []
        targets = ['audio_{}'.format(last_layer_postfix), 'face_{}'.format(last_layer_postfix), 'text_{}'.format(last_layer_postfix), 'int_{}'.format(last_layer_postfix)]
        # targets = ['text_emo', 'face_emo', 'int_emo']
        for idx in range(len(targets)):
            # 7 is the number of classes
            feat_dict.update(
                {'{}_lb'.format(targets[idx]): tf.keras.layers.Input(shape=(num_classes,), name='{}_lb'.format(targets[idx]))})
        ...
        # outs = [base_text_model.output, base_video_model.output, mm_output]
        if num_classes == 6:
            loss_func = MoseiEmotionLoss()
        else:
            loss_func = tf.keras.losses.CategoricalCrossentropy(from_logits=True, label_smoothing=0.0)

        out_mtl = MultiModalLoss(num_outputs=4, loss_function=loss_func,
                                 trainable=True, name='int_{}'.format(last_layer_postfix))(in_out + outs)

        # out_mtl = MultiModalLoss(num_outputs=3, loss_function=tf.keras.losses.CategoricalCrossentropy(from_logits=True,
        #                                                                                               label_smoothing=0.0),
        #                          trainable=True, name='int_emo')(in_out + outs)
        outputs = out_mtl
    else:
        mm_output = layers.Dense(num_classes, activation=act, name='int_{}'.format(last_layer_postfix))(mm_feat)
        # outputs = {'int_{}'.format(last_layer_postfix): mm_output}
        outputs = mm_output

    model = training.Model(feat_dict, outputs, name='MultiModal')
    tf.keras.utils.plot_model(model, './tmp/check_mm.png', show_shapes=True, show_layer_names=True)
    return model
